...
int main(int arg_Num, char *args[]) {//){//
    ...
    while(connectionSocket = accept(serverSocket, (struct sockaddr *) &clnt_Addr, &clnt_Len))
    {
        ...
        if( pthread_create( &thread_id , NULL ,  crt_new_Connect , (void*) &thrd_Args) < 0)
        {
            perror("could not create thread");
            return 1;
        }

        puts("A new thread assigned to the client connection");
    }
    close(serverSocket);
